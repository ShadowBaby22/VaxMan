...
class Game(object):
    ...
    def display_frame(self, screen):
        ...
        if self.game_over:
            ...
            text2 = self.font.render("Ghosts: " + str(self.enemies), True, RED)
            # Put the text on the screen
            screen.blit(text, [120, 20])
            screen.blit(text2, [220, 20])
    ...
    def run_logic(self):
        ...
        if not self.game_over:
            ...
            if len(block_hit_list) > 0:
                self.enemies.explosion = True
                #self.game_over_sound.play()
            #self.game_over = self.player.game_over #Need to change this
            self.enemies.update(self.horizontal_blocks,self.vertical_blocks)

            import time, datetime

            # timer_add = datetime.datetime.utcnow()+datetime.timedelta(0,5)
            # print(timer_add)
            elapsed_time = time.time() - start_time
            print(int(elapsed_time))
            # block_hit_list = pygame.sprite.spritecollide(self.player, self.enemies, True)
            ...
            # HERE is where I need help to double the no of ghosts


            #total_enemies = len(self.enemies)
            #while game_in_progress and len(self.enemies)<=256:
            while len(self.enemies) <= 256:
            #for i in range(total_enemies):
                if elapsed_time > 30:
                    # self.enemies = pygame.sprite.Group()
                    # if total_enemies = 2 , 2+self = 4 = new total enemies
                    enemies_to_add = len(self.enemies)
                    i = 0
                    while i< range(enemies_to_add) and enemies_to_add<256:
                        self.enemies.add(Slime(288, 98, 0, 4))
                        i += 1
                    self.enemies.update(self.horizontal_blocks, self.vertical_blocks)


            print("exits from for loop L201")



            #if elapsed_time > 5 and len(self.enemies) < 16:
                #len(block_hit_list) <= 8 and
                #self.enemies.add(Slime(288, 98, 0, 4))
                #self.enemies.update(self.horizontal_blocks, self.vertical_blocks)
                #print(len(self.enemies))
                #if :
            ...
            #if len(block_hit_list) <= 8 and elapsed_time > 10:
